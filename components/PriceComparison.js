...
export default function PriceComparison() {
  ...
  const loadAnimals = async () => {
    try {
      // Dados reais vazios
      const realData = { animals: [] }
      const response = { ok: true, json: () => Promise.resolve(realData) }
  ...
  const calculateComparison = (market, regional) => {
    ...
    const salesAnalysis = soldAnimals.map((animal) => {
      const weight = estimateWeight(animal.meses, animal.sexo);
      const arrobas = weight / 15;

      let marketPriceAtSale = 0;
      if (animal.meses >= 30) {
        marketPriceAtSale =
          animal.sexo === "Macho"
            ? regional.prices.boi_gordo * arrobas * 0.95 // Assumir preço 5% menor na época
            : regional.prices.vaca_gorda * arrobas * 0.95;
      } else {
        marketPriceAtSale =
          animal.sexo === "Macho"
            ? regional.prices.bezerro_macho * 0.95
            : regional.prices.bezerro_femea * 0.95;
      }

      const marketDifference = animal.valorVenda - marketPriceAtSale;
      const performanceVsMarket =
        marketPriceAtSale > 0
          ? (marketDifference / marketPriceAtSale) * 100
          : 0;

      return {
        ...animal,
        estimatedMarketPrice: marketPriceAtSale,
        marketDifference,
        performanceVsMarket,
      };
    });
